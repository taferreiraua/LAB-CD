...
nao_medalhistas = nao_medalhistas |>
  mutate(Esporte = case_when(Esporte=='Basketball'~'Basquete',
                             ...
                             Esporte=='Judo' ~ 'Judô')) |>
  group_by(Esporte) |>
  mutate(cor = round(cor(Idade, Altura), 4),
...
ggplot(nao_medalhistas, aes(x=Idade, y=Altura)) +
  geom_point(size=4, alpha=.5) +
  geom_smooth(method = 'lm', color='gold', linewidth=2.4) +
  facet_wrap(.~factor(strip.title, stringr::str_sort(unique(strip.title))), ncol=5) +
  geom_richtext(aes(x=15, y=215, label=paste0("<span style='font-size:45pt;'>", 'COR ', 
  ...
  scale_y_continuous(limits=c(150, 215), breaks=seq(160, 200, 10)) +
  scale_x_continuous(breaks=seq(15, 40, 5)) +
  labs(title=paste0("<span style='font-size:140pt;color:#444455;'>", 'CORRELAÇÕES: ', 
                    "<span style='font-size:90pt;color:#BC4B51;'><b>", 'Idade vs. Altura'),
       y='<i>NAO MEDALHISTAS') +
  theme_minimal() +
...
ggsave(units='px', height=2500, width=8000, 
       filename='C:/Users/Thays Ferreira/Documents/LAB/Script/Correlacoes/altura_idade/p1.png')
...
ggplot(medalhistas, aes(x=Idade, y=Altura)) +
  geom_point(size=4, alpha=.5) +
...
ggsave(units='px', height=1700, width=8000, 
       filename='C:/Users/Thays Ferreira/Documents/LAB/Script/Correlacoes/altura_idade/p2.png')
...
ggdraw(plot) +
  draw_image(magick::image_read('C:/Users/Thays Ferreira/Documents/LAB/Script/Correlacoes/altura_idade/p1.png'),
             x=0, y=.207, scale=1) +
  draw_image(magick::image_read('C:/Users/Thays Ferreira/Documents/LAB/Script/Correlacoes/altura_idade/p2.png'),
             x=0, y=-.265, scale=1)
...
ggsave(units='px', height=4000, width=7200, filename='altura_idade.png')